...
async function cadastrarLead({ nome, telefone }) {
    if (!usarPluginRest()) {
        throw new Error('FOLLOW_UP_API_URL e FOLLOW_UP_API_TOKEN sao obrigatorios para cadastrar lead via Vercel.');
    }

    console.log('Cadastrando lead via API REST do follow-up:', telefone);

    return chamarPluginRest('/leads', {
        method: 'POST',
...
module.exports = { cadastrarLead, buscarLeadsPendentesDeHoje, registrarDisparo, pausarLead };
